...
| `feed_death` demo scenario (an instrument that goes `DEGRADED` mid-session) | Time cut | `session/` already computes per-instrument liveness across four states, but no current scenario ever drives an instrument into `DEGRADED` — the one part of the system built specifically to answer "unreliable dependencies" has never been shown working live. This is the next item queued for this pass. |
